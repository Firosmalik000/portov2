...
export const BackgroundBeamsWithCollision = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  ...
  return (
    <div
      ref={parentRef}
      className={cn(
        ' bg-gradient-to-b from-white to-neutral-100 dark:from-black dark:to-neutral-900 relative flex items-center justify-center overflow-hidden',
        // h-screen if you want bigger
        className
      )}
    >
      {beams.map((beam) => (
        <CollisionMechanism key={beam.initialX + 'beam-idx'} beamOptions={beam} containerRef={containerRef as React.RefObject<HTMLDivElement>} parentRef={parentRef as React.RefObject<HTMLDivElement>} />
      ))}
