      ...
      >
        ...
        >
          <Image
            ...
            decoding="sync"
          />
          {current === index && <div className="absolute inset-0 bg-black/60 transition-all duration-1000" />}
        </div>
        ...
        <article className={`relative p-[4vmin] transition-opacity duration-1000 ease-in-out ${current === index ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
          <motion.div className="flex justify-center" initial={{ y: -30 }} animate={current === index ? { y: 0 } : { y: -30 }} transition={{ duration: 0.8, ease: 'easeOut' }}>
